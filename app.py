...
def text_processing(texts, similarity_thresh, min_words, max_words):
    ...
    pie_plot_fig = pie_plot.get_figure()
    return df, pie_plot_fig, pie_plot_fig
    ...
    with gr.Column() as outer_col:
        ...
        with gr.Row() as row2:
            ...
            with gr.Column() as col2:
                ...
                pie_plot = gr.Plot()
                process_button2.click(fn=text_processing, inputs=[textbox, grouping_strength, min_words, max_words], outputs=[df, pie_plot, pie_plot_state])
                save_pie_button = gr.Button(value="Save as Image")
                save_pie_button.click(fn=save_graph, inputs=pie_plot_state)
demo.launch()
